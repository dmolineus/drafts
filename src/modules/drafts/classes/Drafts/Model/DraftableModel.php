...
class DraftableModel extends VersioningModel
{
	 ...
	 * 
	 * @param string|\Model $objModel
	 * @param \Database\Result|Model|null
	 * @param bool $blnVersioning enable versioning
	 * @throws \RuntimeException if table is not draftable
	 */
	public function __construct($objModel, $blnVersioning=false, $strTable=null)
	{
		...
		{
			throw new \RuntimeException('Table "' . $strTable . '" is not draftable.');
		}
	}
	 ...
	 * @param $strState
	 */
	public function toggleState($strState)
	{
		if($this->hasState($strState))
		{
			$this->removeState($strState);
		}
		else {
